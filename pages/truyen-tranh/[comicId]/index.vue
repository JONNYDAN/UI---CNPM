...
import { ComicComments, ComicDetail, Comment, UserInfo } from '@/types';
...
const userInfo = ref<UserInfo>(getUserInfoFromLocalStorage());
...
const toggleFavorite = async () => {
  ...
  try {
    ...
    const response = await axios.post('http://localhost:8000/set_favorite_client', {
...
const onAddDownloadChapter = (chapterId: number) => {
  if (downloadChapters.value.includes(chapterId)) {
    // Xóa chapter khỏi danh sách nếu đã tồn tại
    downloadChapters.value = downloadChapters.value.filter(id => id !== chapterId);
  } else {
    // Thêm chapter vào danh sách
    downloadChapters.value.push(chapterId);
  }
};

// Tải xuống các chương được chọn
const handleDownloadChapters = async () => {

  try {
    isFetching.value = true;

    // Tải xuống từng chương
    for (const chapterId of downloadChapters.value) {
      const downloadUrl = `/download?slug=${comicId}&chapterId=${chapterId}`;
    ...
    // Đặt lại trạng thái sau khi tải xong
    showDownloadModal.value = false;
    downloadChapters.value = [];
  } catch (error) {
    console.error('Lỗi khi tải xuống chương:', error);
    alert('Đã xảy ra lỗi trong quá trình tải xuống. Vui lòng thử lại sau.');
  } finally {
    isFetching.value = false;
  }
};

// Đổi nhóm chương tải xuống (phân trang cho các chương cần tải)
const onChangeChapterDownloadGroup = (idx: number) => {
  currentDownloadChapterPage.value = idx;

  const startIdx = idx === 0 ? 0 : idx * CHAPTER_PER_PAGE + 1;
  const endIdx = (idx + 1) * CHAPTER_PER_PAGE;

  chaptersDownloadSection.value = getChapter(startIdx, endIdx);
};

// Theo dõi trạng thái modal tải xuống để thay đổi giao diện
