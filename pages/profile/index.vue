...
onMounted(async () => {
  if (userInfo.value && userInfo.value.email) {
    try {
      const response  = await axios.get('http://localhost:8000/get_favorite_list', {
        params: {
      ...
      console.log(comicDetails.value); 
...
// Fetching comic details (assuming you have an API to get this data)
const getComicDetails = async () => {
  try {
    const response = await axios.get('http://localhost:8000/get_comic_details');
    comicDetails.value = response.data;
  } catch (error) {
    console.error('Error fetching comic details:', error);
  }
};

// Fetch comic details when the component mounts
getComicDetails();
